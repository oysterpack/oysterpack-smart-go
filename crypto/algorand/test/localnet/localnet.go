...
func AlgodClient(t *testing.T) *algod.Client {
	...
	if err != nil {
		t.Log("failed to connect to algod on localnet", err)
		t.FailNow()
	}
...
func IndexerClient(t *testing.T) *indexer.Client {
	...
	if err != nil {
		t.Log("failed to connect to indexer on localnet", err)
		t.FailNow()
	}
...
func GetPrefundedTestAccounts(t *testing.T) []models.Account {
	...
	if err != nil {
		t.Log("failed to list wallets", err)
		t.FailNow()
	}
	for _, wallet := range listWalletsResponse.Wallets {
		if wallet.Name == DefaultKmdWalletName {
			...
			if err != nil {
				t.Log("failed to init wallet handle for default wallet")
				t.FailNow()
			}
			...
			if err != nil {
				t.Error("failed to list accounts for default wallet", err)
				t.FailNow()
			}
			...
			for _, address := range addresses {
				...
				if err != nil {
					t.Error("failed to retrieve account info", err)
					t.FailNow()
				}
				if account.Amount > 0 {
			...
			if len(accounts) == 0 {
				t.Error("No prefunded accounts were found")
				t.FailNow()
			} else {
